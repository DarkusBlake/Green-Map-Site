...
function getColorByQuality(quality) {
    ...
    switch(value) {
        case 0: return '#605a5a'; // Серый - зелёная пустыня
        case 1: return '#d2222d'; // Красный - плохо
        case 2: return '#d6ce1f'; // Желтый - нормально
        case 3: return '#238823'; // Зеленый - хорошо
        default: return '#cccccc'; // Серый по умолчанию
    }
}
...
function updateStats() {
    ...
    document.getElementById('current-city').textContent = cityData[currentCity].name;
}
    ...
    geoJsonData.features.forEach((feature, index) => {
        ...
        const enrichedFeature = {
            ...
            properties: {
                ...
                quality: parseInt(properties.quality || 0), // По умолчанию "зелёная пустыня"
                // Статистика по паркам (если есть)
...
function renderQuarters() {
            ...
            if (props.great_parks_count > 0 || props.good_parks_count > 0 || props.ok_parks_count > 0) {
                ...
                <div class="parks-info">
                    ...
                    ${props.great_parks_count > 0 ? 
                        `<div>✅ Отличные парки: ${props.great_parks_count} шт. (${Math.round(props.great_parks_area)} м², NDVI: ${props.great_parks_ndvi ? props.great_parks_ndvi.toFixed(3) : '0.000'})</div>` : ''}
                    ${props.good_parks_count > 0 ? 
                        `<div>👍 Хорошие парки: ${props.good_parks_count} шт. (${Math.round(props.good_parks_area)} м², NDVI: ${props.good_parks_ndvi ? props.good_parks_ndvi.toFixed(3) : '0.000'})</div>` : ''}
                    ${props.ok_parks_count > 0 ? 
                        `<div>⚠️ Удовлетворительные парки: ${props.ok_parks_count} шт. (${Math.round(props.ok_parks_area)} м², NDVI: ${props.ok_parks_ndvi ? props.ok_parks_ndvi.toFixed(3) : '0.000'})</div>` : ''}
                </div>`;
            }
            ...
            const popupContent = `
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-card-popup">
                        ...
                        <div class="stat-label-popup">Средний NDVI</div>
                    </div>
                </div>
                ...
                <div class="additional-info">
                    <div>Плотность населения на зелёные зоны: ${props.population_density_per_green_zone.toFixed(3)} чел/га</div>
                </div>
                ` : ''}
    ...
    if (allQuarters.length > 0 && quartersLayer.getBounds().isValid()) {
        ...
        // Оставляем карту в текущем положении
        const cityInfo = cityData[currentCity];
        map.setView(cityInfo.center, cityInfo.zoom);
    }
...
function clearMap() {
    ...
    // Возвращаем карту к центру текущего города
    const cityInfo = cityData[currentCity];
    map.setView(cityInfo.center, cityInfo.zoom);
}
